...
#### Observing Parameter Updates (`configUpdates Flow`)
Observe key changes reactively whenever `activate()` promotes new configuration values:

```kotlin
coroutineScope.launch {
    nonaConfig.configUpdates.collect { changedKeys ->
        if ("welcome_message" in changedKeys) {
            println("Updated message: ${nonaConfig.getString("welcome_message")}")
        }
    }
}
```
...
val settings = NonaConfigSettings.Builder()
    ...
    .setReleaseVersion("1.1.x") // Pin to a specific release line or "active"
    .setPrefix("Checkout:")     // Optional: Fetch only keys matching a specific group prefix
    .build()
...
// Observing Parameter Updates in Swift (AsyncSequence generated by SKIE)
Task {
    for await changedKeys in client.configUpdates {
        if changedKeys.contains("welcome_message") {
            let message = client.getString(key: "welcome_message")
            print("Updated Swift message: \(message)")
        }
    }
}
```
